<div class="phan_slide">
<style type="text/css">
.nav_slide a{ width:183px; height:88px;}

<? 

			$list_hinhanhchay=0;
				$sqltd=$db->select("tgp_gallery_menu","hien_thi=1 and cat='slide_hinh' ","ORDER BY thu_tu");
				while($r2=$db->fetch($sqltd))
				  { $list_hinhanhchay=$list_hinhanhchay.','.$r2['id'];
				  }
				  

$sql=$db->select("tgp_gallery"," hien_thi=1 and cat in(".$list_hinhanhchay.") ","ORDER BY RAND() LIMIT 5");
 while($r2=$db->fetch($sql))
  {
		  $slide=$slide+1;
		  ?>
.page_slide_<?=$slide?>{ background: url(/uploads/gal/thm_<?=$r2['hinh']?>) no-repeat center ; }		  
#slide-<?=$slide?> {
...
<div style="width:100%; position:absolute; z-index:1; top:250px; left:0px;">
	<div style="position:relative;width:984px; margin:auto; display:block"> 
	...
	<div style="width:100%; margin:auto; position:absolute; height:100px; z-index:999; bottom:212px;">		
    
    		<div style="width:984px; margin:auto; text-align:right"><div id="nav_slide" class="nav_slide"></div></div>
